...
class RecipeBook {
    ...
    func parseJSON(_ json: JSON) -> [Recipe]? {
        ...
        if let hits = json["hits"].array {
            for hit in hits {
                if let name = hit["recipe"]["label"].string, let imageUrl = hit["recipe"]["image"].string, let yield = hit["recipe"]["yield"].int, let ingredients = hit["recipe"]["ingredientLines"].arrayObject as? [String], let calories = hit["recipe"]["calories"].float, let totalTime = hit["recipe"]["totalTime"].int {
